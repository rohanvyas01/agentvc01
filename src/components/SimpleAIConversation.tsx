...
export const SimpleAIConversation: React.FC<VideoPersonaConversationProps> = ({
  ...
  const videoSegments = {
    ...
    mid: `/videos/rohan_transition.mp4?cache=transition_${Date.now()}`,   // Mid video
  ...
  const transcribeWithWebSpeech = (): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      recognition.onresult = (event) => {
        ...
        // Build interim transcript from non-final results
        ...
        finalTranscript = completeFinalTranscript;
        interimTranscript = tempInterimTranscript;

        // Show live transcription with accumulated text
  ...
  const saveTranscriptAndAdvance = async (transcribedText: string) => {
    try {
      ...
      if (phase === 'founder_intro') {
        // Save intro transcript
        setIntroTranscript(transcribedText);
        ...
        // Save pitch transcript
        setPitchTranscript(transcribedText);
  ...
  return (
    <div className="max-w-5xl mx-auto">
            ...
            <div className="space-y-6">
              ...
              {isRecording && (
                <div className="space-y-6 text-center">
                  <div className="bg-red-900/50 border border-red-500/60 rounded-xl p-6">
                    <div className="flex items-center justify-center space-x-3 mb-4">
                      <div className="w-4 h-4 bg-red-500 rounded-full animate-pulse"></div>
                      <span className="text-red-300 font-semibold text-lg">🎤 Recording in progress...</span>
                    </div>
              ...
              {isRecording && (
                <div className="space-y-6 text-center">
                  <div className="bg-red-900/50 border border-red-500/60 rounded-xl p-6">
                    <div className="flex items-center justify-center space-x-3 mb-4">
                      <div className="w-4 h-4 bg-red-500 rounded-full animate-pulse"></div>
                      <span className="text-red-300 font-semibold text-lg">🎤 Recording your pitch...</span>
                    </div>

                    {transcript && (
                      <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 max-w-2xl mx-auto max-h-40 overflow-y-auto">
                        <p className="text-sm text-slate-300 italic">"{transcript}"</p>
                      </div>
                    )}
      ...
      {/* Transcript Display - Show both intro and pitch when available */}
      {(introTranscript || pitchTranscript) && (
        <div className="space-y-6 mb-8">
          {/* Introduction Transcript */}
          {introTranscript && (
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
              <h3 className="font-semibold text-slate-100 mb-3 text-lg flex items-center">
                <span className="w-8 h-8 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-full flex items-center justify-center text-white text-sm font-bold mr-3">1</span>
                Your Introduction:
              </h3>
              <div className="bg-slate-700 rounded-lg p-4 border border-slate-600">
                <p className="text-slate-300 italic leading-relaxed">"{introTranscript}"</p>
              </div>
            </div>
          )}
          ...
          {pitchTranscript && (
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
              <h3 className="font-semibold text-slate-100 mb-3 text-lg flex items-center">
                <span className="w-8 h-8 bg-gradient-to-r from-green-500 to-emerald-600 rounded-full flex items-center justify-center text-white text-sm font-bold mr-3">2</span>
                Your Pitch:
              </h3>
              <div className="bg-slate-700 rounded-lg p-4 border border-slate-600">
                <p className="text-slate-300 italic leading-relaxed">"{pitchTranscript}"</p>
              </div>
            </div>
          )}
        </div>
